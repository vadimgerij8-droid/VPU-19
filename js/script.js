...
let pendingPostMediaFile = null; // перейменовано з pendingPostPhotoFile
...
// –– Cloudinary upload (підтримка фото І відео) ––
...
// Зручна обгортка для випадків де нам потрібен лише URL (аватар тощо)
...
// –– Медіа-блок поста (фото або відео) ––
function postMediaHtml(post) {
if (post.mediaType === ‘video’ && post.videoURL) {
return `<video class="post-image post-video" src="${esc(post.videoURL)}" controls playsinline preload="metadata" style="width:100%;border-radius:12px;max-height:520px;object-fit:cover;background:#000;display:block;"> </video>`;
}
if (post.imageURL) {
return `<img class="post-image" src="${esc(post.imageURL)}" alt="Робота" loading="lazy">`;
}
return ‘’;
}
    ...
    <div class="post-card animate-on-scroll" id="post-${post.id}">
      <div class="post-header">
        ...
        <div>
          ...
          <div class="post-date">${dateStr}${isVideo ? ' &nbsp;<span style="font-size:.65rem;background:rgba(201,168,76,.15);color:var(--gold);border-radius:4px;padding:1px 7px;"><i class="fas fa-video"></i> відео</span>' : ''}</div>
        </div>
      </div>
      ${postMediaHtml(post)}
...
html += ` <div class="review-item" id="review-${d.id}"> <div class="review-meta"> <span class="review-author">${esc(r.authorName || 'Анонім')}</span> <div class="review-stars">${stars}</div> <span class="review-date">${dateStr}</span> </div> <div class="review-text">${esc(r.text)}</div> <div class="review-actions"> <button class="like-btn ${likeData.userVote === 'like' ? 'active' : ''}" onclick="handleReviewLike('${d.id}', 'like')"> <i class="far fa-thumbs-up"></i> <span class="like-count" id="like-count-${d.id}">${likeData.likes}</span> </button> <button class="dislike-btn ${likeData.userVote === 'dislike' ? 'active' : ''}" onclick="handleReviewLike('${d.id}', 'dislike')"> <i class="far fa-thumbs-down"></i> <span class="dislike-count" id="dislike-count-${d.id}">${likeData.dislikes}</span> </button> </div> </div>`;
}
...
return `<div class="stars-display">${starsHtml(avg)}</div> <span class="rating-avg">${avg > 0 ? avg : '—'}</span> <span class="rating-count">${d.ratingCount || 0} оцінок</span> <span class="rated-badge" style="margin-left:auto">Ви оцінили</span>`;
}
...
html += `<div class="master-card animate-on-scroll" onclick="navigate('profile','${docSnap.id}')"> <div class="master-card-photo">${u.photoURL ?`<img src="${esc(u.photoURL)}" loading="lazy">`: '<i class="fas fa-user-tie" style="font-size:3rem;color:var(--gray-mid)"></i>'}</div> <div class="master-card-info"> <div class="master-card-name">${esc(u.name || u.email)}</div> <div class="master-card-role"><i class="fas fa-${u.role === 'admin' ? 'crown' : 'cut'}"></i> ${u.role === 'admin' ? 'Адміністратор' : 'Майстер'}</div> <div class="master-card-stats"><span><i class="far fa-image"></i> ${postsSnap.size} робіт</span>${u.phone ?`<span><i class="fas fa-phone-alt"></i> ${esc(u.phone)}</span>` : ''}</div> <div class="master-card-rating-row"> <div class="stars-display">${starsHtml(avg)}</div> <span class="rating-avg">${avg > 0 ? avg : '—'}</span> <span class="rating-count">${rCount} оцінок</span> </div> </div> </div>`;
}
...
return `<div class="profile-review-card"> <div class="profile-review-card-meta"> <div class="profile-review-card-info"> <div class="profile-review-card-author">${esc(r.authorName || 'Анонім')}</div> <div class="profile-review-card-post">До: ${esc(postCaption.length > 40 ? postCaption.slice(0,40)+'…' : postCaption)}</div> </div> <div class="profile-review-card-date">${dateStr}</div> </div> <div style="display:flex;gap:3px;margin-bottom:8px">${stars}</div> <div class="profile-review-card-text">${esc(r.text)}</div> </div>`;
}).join(’’) + ‘</div>’;
}
...
return `<div class="haircut-rating-item"> ${isVideo ? `<video class="haircut-rating-thumb" src="${esc(p.videoURL)}" muted playsinline preload="metadata" style="object-fit:cover;"></video>`: p.imageURL ?`<img class="haircut-rating-thumb" src="${esc(p.imageURL)}" loading="lazy" onerror="this.style.display='none'">` : '<div class="haircut-rating-thumb"><i class="fas fa-image" style="font-size:2rem;color:var(--gray-mid)"></i></div>' } <div class="haircut-rating-info"> <div class="haircut-rating-caption">${esc(p.caption || 'Без опису')}${isVideo ? ' <span style="font-size:.6rem;color:var(--gold)"><i class="fas fa-video"></i></span>' : ''}</div> <div class="haircut-rating-stats"> <div style="display:flex;gap:3px">${stars}</div> <span class="haircut-rating-avg">${pAvg || '—'}</span> <span class="haircut-rating-count">${p.ratingCount || 0} оцінок</span> </div> </div> </div>`;
}).join(’’) + ‘</div>’;
}

```
// Сітка постів у профілі — відео показуємо з іконкою відтворення
const postsGridHtml = posts.length
  ? `<div class="profile-posts-grid">
      ${posts.map(p => {
        const isVideo = p.mediaType === 'video' && p.videoURL;
        const thumb = isVideo
          ? `<video src="${esc(p.videoURL)}" muted playsinline preload="metadata" style="width:100%;height:100%;object-fit:cover;display:block;"></video>
             <div class="profile-post-video-icon"><i class="fas fa-play"></i></div>`
          : `<img src="${esc(p.imageURL)}" loading="lazy" onerror="this.src='https://placehold.co/400?text=No+Image'">`;
        return `<div class="profile-post-item" style="position:relative;">
          ${thumb}
          <div class="profile-post-overlay"><i class="fas fa-star"></i> ${p.ratingCount > 0 ? (p.ratingSum/p.ratingCount).toFixed(1) : '—'}</div>
        </div>`;
      }).join('')}
    </div>`
  : '<p style="color:var(--gray-mid);font-size:.82rem">Ще немає опублікованих робіт</p>';
...
if (pendingProfilePhotoFile) {
const photoURL = await uploadImageToCloudinary(pendingProfilePhotoFile, `avatars/${currentUser.uid}`);
...
const photoURL = await uploadImageToCloudinary(pendingMasterPhotoFile, `avatars/${editingMasterId}`);
updates.photoURL = photoURL;
}
...
let photoURL = null;
if (pendingMasterPhotoFile) { photoURL = await uploadImageToCloudinary(pendingMasterPhotoFile, `avatars/${cred.user.uid}`); }
...
area.style.position = ‘’;
area.innerHTML = `<i class="fas fa-photo-video" style="font-size:2rem;color:var(--gray-mid);margin-bottom:8px"></i> <input type="file" id="postPhotoInput" accept="image/*,video/*"> <div class="upload-hint">Натисніть або перетягніть <strong>фото</strong> або <strong>відео</strong></div>`;
bindPostMediaInput();
}

function bindPostMediaInput() {
...
if (isVideo) {
  area.innerHTML = `
    <video src="${url}"
      style="width:100%;max-height:280px;border-radius:10px;object-fit:cover;display:block;"
      muted autoplay loop playsinline>
    </video>
    <div style="font-size:.7rem;color:var(--gold);text-align:center;margin-top:8px;"><i class="fas fa-video"></i> Відео обрано</div>
    <label style="cursor:pointer;font-size:.7rem;color:var(--gray-mid);text-decoration:underline;margin-top:4px;display:block;text-align:center;">
      Змінити файл
  ...
  area.innerHTML = `
    <label style="cursor:pointer;font-size:.7rem;color:var(--gray-mid);text-decoration:underline;position:absolute;bottom:10px;right:14px;">
      Змінити файл
      <input type="file" id="postPhotoInput" accept="image/*,video/*" style="display:none;">
    </label>
  `;
...
const { url, type } = await uploadToCloudinary(pendingPostMediaFile, `posts/${currentUser.uid}`);

```
const postData = {
  authorId: currentUser.uid,
  caption,
  mediaType: type,           // 'image' або 'video'
