...
return ` <div class="post-media-carousel" data-post-id="${postId}"> <div class="carousel-track">${slides}</div> <button class="carousel-nav carousel-prev" aria-label="Попередній"><i class="fas fa-chevron-left"></i></button> <button class="carousel-nav carousel-next" aria-label="Наступний"><i class="fas fa-chevron-right"></i></button> <div class="carousel-dots">${dots}</div> <div class="carousel-counter">1/${media.length}</div> </div>`;
}
...
tr.innerHTML = `<td><div class="table-avatar">${u.photoURL ?`<img src="${esc(u.photoURL)}">` : '<i class="fas fa-user"></i>'}</div></td> <td>${esc(u.name || '')}</td> <td>${esc(u.email)}</td> <td>${u.role}</td> <td>${u.status || 'master'}</td> <td class="table-actions"> <button class="btn-secondary btn-sm" onclick="editMaster('${docSnap.id}')"><i class="fas fa-edit"></i></button> <button class="btn-danger btn-sm" onclick="changeRole('${docSnap.id}', '${u.role === 'admin' ? 'master' : 'admin'}')"><i class="fas fa-exchange-alt"></i></button> <button class="btn-danger btn-sm" onclick="deleteMaster('${docSnap.id}')"><i class="fas fa-trash"></i></button> </td>`;
